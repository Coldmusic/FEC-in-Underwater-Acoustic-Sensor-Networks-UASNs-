...
SUBROUTINE INIT
  ...
  IF ( IAllocStat /= 0 ) &
       CALL ERROUT( PRTFile, 'F', 'KRAKEN - INIT', 'Insufficient memory to allocate B1, B2, B3, B4 vectors: Reduce mesh.' )
...
SUBROUTINE SOLVE2
  ...
  DO mode = 1, M
     ...
     CALL ZSECX( x, Tolerance, Iteration, MaxIteration, ErrorMessage )

     IF ( ErrorMessage /= ' ' ) THEN
        WRITE( PRTFile, * ) 'ISet, mode = ', ISet, mode
        CALL ERROUT( PRTFile, 'W', 'KRAKEN-ZSECX', ErrorMessage )
...
SUBROUTINE SOLVE3
  ...
  DO mode = 1, M
     ...
     CALL ZSECX( x, Tolerance, IT, MaxIT, ErrorMessage )  ! Use the secant method to refine the eigenvalue
...
SUBROUTINE AcousticLayers( x, F, G, IPower )
  ...
  REAL (KIND=8), INTENT(IN) :: x
  REAL (KIND=8)             :: h2k2, F, G, p0, p1, p2, rhoMedium
...
SUBROUTINE VECTOR( FileRoot )
  ...
  IF ( IAllocStat /= 0 ) CALL ERROUT( PRTFile, 'F', 'KRAKEN - VECTOR', 'Insufficient memory: Reduce mesh.' )
...
!!$     end if
     ...
     IF ( IERR /= 0 ) THEN
        ...
        CALL ERROUT( PRTFile, 'W', 'KRAKEN-SINVIT', 'Inverse iteration failed to converge' )
     ENDIF
...
SUBROUTINE Normalize( Phi, ITP, NTotal1, x )
  ...
  USE krakmod
  IMPLICIT NONE
  INTEGER           :: IPower, ITP, J, J1, L, L1, Medium, NTotal1
  REAL     (KIND=8) :: Phi( NTotal1 ), Slow, rhoMedium, rho_omega_h2, ScaleFactor, SqNorm, RN, x, x1, x2, F1, F2, G1, G2, &
                       detadx, drhodx
  ...
  IF ( BotOpt( 1 : 1 ) == 'A' ) THEN
     Del  = -0.5D0 * ( omega2 / HSBot%cP ** 2 - DBLE( omega2 / HSBot%cP ** 2 ) ) / &
                 SQRT( x                      - DBLE( omega2 / HSBot%cP ** 2 ) )
...
SUBROUTINE ScatterLoss( PERK, Phi, x )
  ...
  REAL     (KIND=8) :: Phi( * ), omega, rho1, rho2, rhoInside, cInside, dPhidz, x, h2
  COMPLEX  (KIND=8) :: CImped, kx, Twersky, PERK, eta1Sq, eta2Sq, KupIng, U, PhiC
