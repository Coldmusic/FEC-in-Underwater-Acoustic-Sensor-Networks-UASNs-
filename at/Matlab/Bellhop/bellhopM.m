...
SrcBmPat = readpat( sbpfil, Beam.RunType(3:3) );   % optionally read source beam pattern
...
if ( ismember( 'A', upper( Beam.RunType(1:1) ) ) )
    ...
    p = zeros( 1, Pos.Nsd, Pos.Nrd, Pos.Nrr );
end
...
for is = 1 : Pos.Nsd
    ...
    if ( Beam.RunType(1:1) == 'A' )   % arrivals calculation
        ...
        p( 1, is, :, : ) = scalep( Dalpha, c, Pos.r.range, U, Beam.RunType, Bdry.Top.Opt, freq );
        PlotType = 'rectilin  ';
        atten = 0;
