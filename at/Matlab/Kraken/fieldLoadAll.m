...
if ( NProf == 1 )   % Range-independent case
   ...
   p = evalri( C, phiR( :, 1 : min( MLimit, Modes.M ) ), R, Modes.k( 1 : min( MLimit, Modes.M ) ), Opt, Comp );
else
   if ( Opt(2:2) == 'C' )   % Range-  dependent case
      % Coupled mode theory
      p = evalcmLoadAll( fileroot, Pos, Dwedge, rd, Nrd, R, Nr, MLimit, Opt );
   else
      % Adiabatic mode theory
      p = evalad( rProf, NProf,      C, phiR, rd, Nrd, R, Nr, M, Opt );
   end
end
