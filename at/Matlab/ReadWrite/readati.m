...
if ( TopATI == '*' )
    ...
    if ( strcmp( atifil, 'ATIFIL' ) == 0 && isempty( findstr( atifil, '.ati' ) ) )
        atifil = [ atifil '.ati' ]; % append extension
    end
    
    fid = fopen( atifil, 'r' );
    if ( fid == -1 )
        errordlg( 'ATIFIL does not exist', 'readati' )
        error(    'ATIFIL does not exist', 'readati' )
    end
