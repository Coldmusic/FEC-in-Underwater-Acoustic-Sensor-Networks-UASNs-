...
if ( BotBTY == '*' )
    ...
    if ( strcmp( btyfil, 'BTYFIL' ) == 0 && isempty( findstr( btyfil, '.bty' ) ) )
        btyfil = [ btyfil '.bty' ]; % append extension
    end
    
    fid = fopen( btyfil, 'r' );
    if ( fid == -1 )
        errordlg( 'Bathymetry file does not exist', 'readbty' )
        error(    'Bathymetry file does not exist', 'readbty' )
    end
